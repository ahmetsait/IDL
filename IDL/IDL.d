					...
					for(i++; i < tokens.length; i++)
					{
						switch(tokens[i])
						{
							...
							case "type:":
								if(obji < 0)
									continue Lloop1;
								objs[obji].type = cast(ObjectType)tokens[++i].to!int;
								break;
							case "file:":
								if(obji < 0)
									continue Lloop1;
	...
	try
	{
		try
		{
			...
			version(all)
			{
				...
				//MessageBoxW(hMainWindow, utf.toUTF16z(wtext(*game)), "[IDL.dll] Debug", MB_OK);
				if(game.state == 2 && game.files != null)
				{
					sFileManager* fm = new sFileManager;
					memset(fm, 0, sFileManager.sizeof);
					if(ReadProcessMemory(hProc, game.files, fm, sFileManager.sizeof, null) == TRUE)
					{
						sDataFile* df = new sDataFile;
						if(ReadProcessMemory(hProc, fm.datas[0], df, sDataFile.sizeof, null) == TRUE)
							MessageBoxW(hMainWindow, utf.toUTF16z(
									format("x=%d  y=%d  w=%d  h=%d", 
										df.frames[30].bdy_x, df.frames[30].bdy_y, df.frames[30].bdy_w, df.frames[30].bdy_h)), 
								"[IDL.dll] Debug", MB_OK);
					}
				}
			}
									...
									Lcswitch3:
										switch(tokens[i])
										{
											...
											case "hit_Dj:":
												...
												break;
											case "hit_ja:":
												DataFile.frames[frameId].hit_ja = tokens[++i].to!int;
												break;
											case "mp:":
