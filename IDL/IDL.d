...
version(DLL)
{
	...
	export extern(C) BOOL SuspendThreadList(int* threadIds, int length, BOOL bSuspend) nothrow @nogc @system
	{
		...
		bool rollback = false;
		BOOL ret = TRUE;
		for(int i = 0; rollback ? i >= 0 : i < length; rollback ? i-- : i++)
		{
...
//		SendMessageA(window, WM_MOUSEMOVE, 15073680, 1); // Not working
	...
	int ReadDataTxt(wchar* dataTxtFile, int length, ref DataTxt dataTxt, HWND hMainWindow)
	{
		try
		{
						...
						for(i++; i < tokens.length; i++)
						{
							switch(tokens[i])
							{
								case "id:":
									objs ~= ObjectData();
									obji++;
									objs[obji].id = tokens[++i].to!int;
									break;
								case "type:":
									if(obji < 0)
										continue Lloop1;
									objs[obji].type = tokens[++i].to!ObjectType;
									break;
								case "file:":
									if(obji < 0)
										continue Lloop1;
									i++;
									objs[obji].file = cast(wchar*)malloc(wchar.sizeof * (tokens[i].length + 1));
									foreach(j, ch; tokens[i])
										objs[obji].file[j] = ch;
									objs[obji].file[tokens[i].length] = '\0';
									break;
								case "<object_end>":
									dataTxt.objects = cast(ObjectData*)malloc(ObjectData.sizeof * objs.length);
									memcpy(dataTxt.objects, objs.ptr, objs.length * ObjectData.sizeof);
									dataTxt.objCount = objs.length;
									break Lloop1;
								default:
									//ignore
									break;
						...
						for(i++; i < tokens.length; i++)
						{
							switch(tokens[i])
							{
								case "id:":
									bgs ~= BackgroundData();
									bgi++;
									bgs[bgi].id = tokens[++i].to!int;
									break;
								case "file:":
									if(bgi < 0)
										continue Lloop2;
									i++;
									bgs[bgi].file = cast(wchar*)malloc(wchar.sizeof * (tokens[i].length + 1));
									foreach(j, ch; tokens[i])
										bgs[bgi].file[j] = ch;
									bgs[bgi].file[tokens[i].length] = '\0';
									break;
								case "<background_end>":
									dataTxt.backgrounds = cast(BackgroundData*)malloc(BackgroundData.sizeof * bgs.length);
									memcpy(dataTxt.backgrounds, bgs.ptr, bgs.length * BackgroundData.sizeof);
									dataTxt.bgCount = bgs.length;
									break Lloop2;
								default:
									//ignore
									break;
	...
	export extern(C) int InstantLoad(wchar* data, int length, int procId, DataType dataType, 
		int objId, ObjectType objType, HWND hMainWindow) @system
	{
		try
		{
			...
			if(hProc == INVALID_HANDLE_VALUE)
				throw new Exception("Could not open process (Maybe you need Administrator privileges?)");
						...
						Lcswitch1:
							switch(tokens[i])
							{
								...
								Lcloop2:
									for(i++; i < tokens.length; i++)
									{
										if(tokens[i].length >= 4 && tokens[i][0 .. 4] == "file")
